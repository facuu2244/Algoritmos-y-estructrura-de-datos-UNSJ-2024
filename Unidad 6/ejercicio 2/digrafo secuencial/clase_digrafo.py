...
class digrafo:
    ...
    def __init__(self, vertices, aristas):
        self.__matriz_adyacencia=np.zeros((len(vertices), len(vertices)), dtype=int)
        self.__vertices=vertices
        self.__tiempo=0
        
        for a in aristas:
            self.__matriz_adyacencia[a[0]][a[1]]=1
    ...
    def REA(self, v_origen):
        ...
        while cola_secuencial.vacia()!=True:
            ...
            for u in range(len(self.__vertices)):
                if self.__matriz_adyacencia[v][u]==1 and vertices_visitados[u]==float("inf"):
        ...
        for i in range(len(self.__vertices)):
            for j in range(len(self.__vertices)):
                if self.__matriz_adyacencia[i][j] == 1:
                    transpuesto.__matriz_adyacencia[j][i] = 1
        return transpuesto
        ...
        for i in range(len(self.__vertices)):
            for j in range(len(self.__vertices)):
                if self.__matriz_adyacencia[i][j] == 1 or self.__matriz_adyacencia[j][i] == 1:
                    no_dirigido.__matriz_adyacencia[i][j] = 1
                    no_dirigido.__matriz_adyacencia[j][i] = 1
    ...
    def aciclico_visita(self, v, descubiertos):
        ...
        #recorro los adyacentes de "v"
        for u in range(len(self.__vertices)):
            if self.__matriz_adyacencia[v][u]==1:
                if descubiertos[u] == 0:
    ...
    def grado_salida(self, vertice):#veo la fila del vertice
        ...
        for v in range(len(self.__vertices)):
            if self.__matriz_adyacencia[indice][v]!=0:
                cont+=1
    ...
    def grado_entrada(self, vertice):#veo la columna del vertice
        ...
        indice=self.buscar_indice(vertice)
        for v in range(len(self.__vertices)):
            if self.__matriz_adyacencia[v][indice]!=0:
                cont+=1
