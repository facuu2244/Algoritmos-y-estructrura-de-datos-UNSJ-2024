...
class arbol:
    ...
    def buscar(self, elemento):#recibe clave a buscar
        if self.__raiz!=None:
            ...
            print(f"Raiz busqueda: {aux_raiz.dar_elemento()}")
    ...
    def insertar_raiz(self, raiz):#se envia el nodo por parametro
        self.__raiz=raiz
    ...
    def camino(self, comienzo, final):#recibe claves de nodos comienzo y final
        if self.__raiz!=None:
            ...
            print(f"Inicio nodo: {nodo_comienzo.dar_elemento()}")  # Debug
            print(f"Fin nodo: {nodo_final.dar_elemento()}")  # Debug
    ...
    def mostar(self, aux, i=0):
        x=" "
        for _ in range(i):
            x+="    "
        if aux!=None:
            i+=1
            self.mostar(aux.dar_izq(), i)
            print(f"{x}{aux.dar_elemento()}")
            self.mostar(aux.dar_der(), i)